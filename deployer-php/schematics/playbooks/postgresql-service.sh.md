# Schematic: postgresql-service.sh

> Auto-generated schematic. Last updated: 2025-12-19

## Overview

Remote bash playbook that controls the PostgreSQL service lifecycle (start/stop/restart) via systemctl on Ubuntu/Debian servers. Invoked by PHP commands through SSH to manage database service state with verification polling.
...
|----------|-----------|-------------|--------------|
| Line 34-35 | `$DEPLOYER_ACTION` is `start` or `restart` | Run systemctl action, verify active | Check next case |
...
flowchart TD
    subgraph "PHP Commands"
        start[PostgresqlStartCommand]
        stop[PostgresqlStopCommand]
        restart[PostgresqlRestartCommand]
    end
    ...
    subgraph "postgresql-service.sh"
        main["main()"]
        exec["execute_action()"]
...
sequenceDiagram
    ...
    Cmd->>Trait: executePlaybookSilently(server, 'postgresql-service', [...])
    ...
    alt DEPLOYER_ACTION = start/restart
        Script->>Sys: run_cmd systemctl start/restart postgresql
        Sys->>DB: Start/Restart service
        loop Until active or timeout (10s)
            Script->>Sys: systemctl is-active --quiet postgresql
        end
    else DEPLOYER_ACTION = stop
        Script->>Sys: run_cmd systemctl stop postgresql
        Sys->>DB: Stop service
...
|------|---------------|-------------|
| `app/Console/Postgresql/PostgresqlStartCommand.php` | Command | Invokes with `DEPLOYER_ACTION=start` |
| `app/Console/Postgresql/PostgresqlStopCommand.php` | Command | Invokes with `DEPLOYER_ACTION=stop` |
| `app/Console/Postgresql/PostgresqlRestartCommand.php` | Command | Invokes with `DEPLOYER_ACTION=restart` |
...
| `playbooks/mariadb-service.sh` | Pattern | Parallel implementation for MariaDB (identical structure) |
...
| Effect | Description |
|--------|-------------|
| Service state change | PostgreSQL service started, stopped, or restarted via systemctl |
| Process signals | systemctl sends appropriate signals to PostgreSQL daemon |
...
### Comparison with mysql-service.sh and mariadb-service.sh

This playbook shares identical structure with `mysql-service.sh` and `mariadb-service.sh`, differing only in the service name (`postgresql` vs `mysqld` vs `mariadb`). All three handle the same three actions with the same verification logic.
