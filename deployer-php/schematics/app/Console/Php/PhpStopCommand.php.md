# Schematic: PhpStopCommand.php

> Auto-generated schematic. Last updated: 2025-12-19

## Overview

PhpStopCommand stops PHP-FPM service(s) on a remote server. It can stop a specific PHP version or all installed versions. Uses the `php-service.sh` playbook with `stop` action.
...
| Method | Purpose |
|--------|---------|
| `execute()` | Main command execution - stop PHP-FPM service(s) |

### Execution Flow

1. Call `parent::execute()` to display env/inventory status
2. Display heading "Stop PHP-FPM Service"
...
| `--version` provided and valid | Stop single version |
| `--version` omitted | Stop all versions |
...
| `Command::SUCCESS` | PHP-FPM stopped successfully |
...
```mermaid
sequenceDiagram
    participant Cmd as PhpStopCommand
    ...
    SSH->>Remote: php-service.sh DEPLOYER_ACTION=stop
    Remote-->>SSH: systemctl stop php8.3-fpm php8.4-fpm
...
| Effect | Description |
|--------|-------------|
| Remote | Stops PHP-FPM service(s) via `systemctl stop php{X.Y}-fpm` |

## Notes

- Stopping PHP-FPM will make all PHP sites on the server non-functional
- If `--version` is omitted, ALL installed PHP versions are stopped
- Version display adapts: "PHP 8.4-FPM" for single, "PHP-FPM (8.3, 8.4)" for multiple
- Playbook verifies each service is stopped after stop with 10-second timeout
