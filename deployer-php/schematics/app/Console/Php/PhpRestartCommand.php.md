# Schematic: PhpRestartCommand.php

> Auto-generated schematic. Last updated: 2025-12-19

## Overview

PhpRestartCommand restarts PHP-FPM service(s) on a remote server. It can restart a specific PHP version or all installed versions. Uses the `php-service.sh` playbook with `restart` action.
...
| Method | Purpose |
|--------|---------|
| `execute()` | Main command execution - restart PHP-FPM service(s) |

### Execution Flow

1. Call `parent::execute()` to display env/inventory status
2. Display heading "Restart PHP-FPM Service"
...
| `--version` provided and valid | Restart single version |
| `--version` omitted | Restart all versions |
...
| `Command::SUCCESS` | PHP-FPM restarted successfully |
...
flowchart TD
    ...
    B --> C[h1 'Restart PHP-FPM Service']
    ...
    P --> Q{Success?}
    Q -->|No| R[nay 'Failed to restart']
    R --> F

    Q -->|Yes| S[yay 'Restarted']
    S --> T[commandReplay]
    T --> U[Return SUCCESS]
```
...
```mermaid
sequenceDiagram
    participant Cmd as PhpRestartCommand
    ...
    SSH->>Remote: php-service.sh DEPLOYER_ACTION=restart
    Remote-->>SSH: systemctl restart php8.3-fpm php8.4-fpm
...
| Effect | Description |
|--------|-------------|
| Remote | Restarts PHP-FPM service(s) via `systemctl restart php{X.Y}-fpm` |

## Notes

- Restart is useful after php.ini changes or to clear opcache
- If `--version` is omitted, ALL installed PHP versions are restarted
- Version display adapts: "PHP 8.4-FPM" for single, "PHP-FPM (8.3, 8.4)" for multiple
- Playbook verifies each service is active after restart with 10-second timeout
- Brief downtime occurs during restart (typically < 1 second per version)
