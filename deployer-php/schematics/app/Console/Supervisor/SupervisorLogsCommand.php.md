# Schematic: SupervisorLogsCommand.php

> Auto-generated schematic. Last updated: 2025-12-19

## Overview

Displays supervisord service logs and per-site program logs from a selected server. Uses LogsTrait methods for journal and file log retrieval with configurable line count and error highlighting.
...
1. **Display heading** - Show "Supervisor Logs" header via `h1()`
...
5. **Retrieve service logs** - Call `retrieveJournalLogs()` for supervisor systemd unit
6. **Retrieve program logs** - Iterate sites on server via `$this->sites->findByServer()`, call `retrieveFileLogs()` for each supervisor program
...
```mermaid
flowchart TD
    subgraph Command["SupervisorLogsCommand"]
        Execute[execute]
    end
    ...
    subgraph RemoteServer["Remote Server"]
        Journalctl[journalctl -u supervisor]
        TailLogs[tail program logs]
    end
...
| `app/Repositories/SiteRepository.php` | Data | Site inventory access for program log iteration |
| `app/DTOs/ServerDTO.php` | Data | Server connection details |
| `app/DTOs/SiteDTO.php` | Data | Site domain and supervisor programs |
| `app/DTOs/SupervisorDTO.php` | Data | Supervisor program name for log path |
...
| `/var/log/supervisor/` | Log | Supervisor program logs on remote server |
...
| Console | "Supervisor Logs" heading | formatted text |
| Console | Server details display | formatted text |
| Console | Supervisor service logs (journalctl) | formatted text |
| Console | Per-site program logs (tail) | formatted text |
...
- Uses LogsTrait's `retrieveJournalLogs()` for supervisor systemd service logs
- Uses LogsTrait's `retrieveFileLogs()` to show per-site program logs at `/var/log/supervisor/{domain}-{program}.log`
- Iterates all sites on server via `$this->sites->findByServer()`, then all supervisors per site via `$site->supervisors`
