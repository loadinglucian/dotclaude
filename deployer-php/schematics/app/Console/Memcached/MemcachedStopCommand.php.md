# Schematic: MemcachedStopCommand.php

> Auto-generated schematic. Last updated: 2025-12-19

## Overview

`MemcachedStopCommand` stops the Memcached service on a remote server. It executes the `memcached-service` playbook with `DEPLOYER_ACTION=stop`, then verifies the service has stopped before reporting success.
...
| `execute()` | protected | Main execution - selects server, stops service |
...
1. execute()
   |
   +-- Display heading "Stop Memcached Service"
   |
   ...
   |   |-- True: return FAILURE
   |
   +-- executePlaybookSilently(server, 'memcached-service', 'Stopping...', {DEPLOYER_ACTION: 'stop'})
   |
   +-- Check: is_int($result)
   |   |-- True: nay("Failed to stop Memcached service"), return FAILURE
   |
   +-- yay("Memcached service stopped")
   |
   +-- commandReplay('memcached:stop', ['server' => $server->name])
   |
   +-- return SUCCESS
```
...
```mermaid
flowchart TD
    subgraph Command["MemcachedStopCommand"]
        execute["execute()"]
    end
    ...
    subgraph Remote["Remote Server"]
        Systemctl["systemctl stop memcached"]
    end

    execute --> ServersTrait
    execute --> PlaybooksTrait
    PlaybooksTrait -->|DEPLOYER_ACTION=stop| MemcachedService
...
|--------|----------|---------|
| Memcached service stopped | Remote server | Successful execution |

## Notes

### Service Verification

The `memcached-service.sh` playbook includes a 10-second polling loop (`verify_service_stopped`) that confirms Memcached has stopped before returning success.

### Pattern Consistency

Matches structure of `MysqlStopCommand`, `MariadbStopCommand`.
