# Schematic: PostgresqlStartCommand.php

> Auto-generated schematic. Last updated: 2025-12-19

## Overview

CLI command that starts the PostgreSQL service on a remote server. Uses `ServersTrait::selectServerDeets()` for server selection with SSH validation, and `PlaybooksTrait::executePlaybookSilently()` to execute the `postgresql-service` playbook with `DEPLOYER_ACTION=start`. The playbook uses systemctl to start the service and verifies it becomes active.
...
1. **Display heading** - Output "Start PostgreSQL Service" heading via `h1()`
...
4. **Execute playbook** - Call `executePlaybookSilently()` (PlaybooksTrait) with:
   ...
   - Playbook name: `postgresql-service`
   - Status message: "Starting PostgreSQL service..."
...
6. **Success message** - Display "PostgreSQL service started" via `yay()`
...
|----------|-----------|--------|
| Line 52 | `is_int($server) \|\| null === $server->info` | Return `FAILURE` (server selection or info retrieval failed) |
| Line 69 | `is_int($result)` | Display error message, return `FAILURE` (playbook execution failed) |
...
```mermaid
flowchart TD
    subgraph Command["PostgresqlStartCommand"]
        A[execute] --> B[h1: Start PostgreSQL Service]
        ...
        G -->|Yes| I[yay: PostgreSQL service started]
        I --> J[commandReplay]
        J --> K[Return SUCCESS]
    end
    ...
    subgraph PlaybooksTrait["PlaybooksTrait"]
        F --> PT1[Read postgresql-service.sh]
    ...
    subgraph External["External Systems"]
        ...
        Server --> Systemctl[systemctl start postgresql]
    end
```
...
sequenceDiagram
    participant User
    participant Cmd as PostgresqlStartCommand
    participant ST as ServersTrait
    participant PT as PlaybooksTrait
    participant SSH as SshService
    participant Server as Remote Server
    participant Script as postgresql-service.sh

    User->>Cmd: postgresql:start [--server=name]
    Cmd->>Cmd: h1("Start PostgreSQL Service")
    ...
    Cmd->>PT: executePlaybookSilently(server, postgresql-service, vars)
    PT->>SSH: Execute heredoc with DEPLOYER_ACTION=start
    SSH->>Server: SSH connection
    Server->>Script: Execute postgresql-service.sh
    Script->>Script: run_cmd systemctl start postgresql
    ...
    Cmd->>Cmd: yay("PostgreSQL service started")
...
|------|---------------|-------------|
| `playbooks/postgresql-service.sh` | Playbook | Executed with `DEPLOYER_ACTION=start` to start PostgreSQL |
...
|-------------|------|------|
| Console | Start PostgreSQL Service heading | Formatted text |
| Console | Server details | Formatted key-value pairs |
| Console | Spinner during execution | "Starting PostgreSQL service..." |
| Console | Success message | "PostgreSQL service started" |
| Console | Error message (on failure) | "Failed to start PostgreSQL service" |
...
| Remote playbook | Executes `postgresql-service.sh` playbook with `DEPLOYER_ACTION=start` |
| Service state change | Starts PostgreSQL systemd service on remote server |
| Temp file | Creates and cleans up `/tmp/deployer-output-*.yml` on remote server |

## Environment Variables

Injected into `postgresql-service.sh` via PlaybooksTrait:
...
| `DEPLOYER_ACTION` | `start` | Action for postgresql-service.sh |
...
The `postgresql-service.sh` playbook:

1. Validates required environment variables
2. Calls `systemctl start postgresql` via `run_cmd` (permission-aware)
3. Polls `systemctl is-active postgresql` for up to 10 seconds
4. Returns `{status: success}` on success, exits with code 1 on failure

### Related Commands

- `postgresql:stop` - Identical structure, uses `DEPLOYER_ACTION=stop`
- `postgresql:restart` - Identical structure, uses `DEPLOYER_ACTION=restart`
- `postgresql:logs` - Different pattern, uses `LogsTrait` for log retrieval
- `postgresql:install` - Different playbook (`postgresql-install`), more complex flow
- `mysql:start` - Identical structure for MySQL service
