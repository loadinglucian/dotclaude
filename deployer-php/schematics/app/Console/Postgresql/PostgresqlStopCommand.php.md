# Schematic: PostgresqlStopCommand.php

> Auto-generated schematic. Last updated: 2025-12-19

## Overview

CLI command that stops the PostgreSQL service on a remote server. Uses `ServersTrait::selectServerDeets()` for server selection with SSH validation, then executes the `postgresql-service` playbook with `DEPLOYER_ACTION=stop` to stop the PostgreSQL systemd service. Part of the PostgreSQL service control command family (start, stop, restart, status, install).
...
1. **Display heading** - Output "Stop PostgreSQL Service" heading via `h1()`
...
4. **Execute playbook** - Call `executePlaybookSilently()` (PlaybooksTrait) with:
   ...
   - Status message: "Stopping PostgreSQL service..."
...
| Line 69 | `is_int($result)` | Display error, return `FAILURE` (playbook execution failed) |
...
```mermaid
flowchart TD
    subgraph Command["PostgresqlStopCommand"]
        A[execute] --> B[h1: Stop PostgreSQL Service]
        ...
        G -->|Yes| I[yay: PostgreSQL service stopped]
        I --> J[commandReplay]
        J --> K[Return SUCCESS]
    end
    ...
    subgraph PlaybooksTrait["PlaybooksTrait"]
        ...
        PT1 --> PT2[Read postgresql-service.sh]
    ...
    subgraph External["External Systems"]
        ...
        Server --> Systemd[systemctl stop postgresql]
    end
```
...
sequenceDiagram
    participant User
    participant Cmd as PostgresqlStopCommand
    ...
    User->>Cmd: postgresql:stop [--server]
    Cmd->>Cmd: h1("Stop PostgreSQL Service")
    ...
    Cmd->>PT: executePlaybookSilently(postgresql-service, DEPLOYER_ACTION=stop)
    PT->>SSH: Execute script with heredoc
    SSH->>RS: systemctl stop postgresql
    RS->>RS: verify_service_stopped()
    RS-->>SSH: YAML: status=success
    SSH-->>PT: Parsed result
    PT-->>Cmd: Result array

    Cmd->>Cmd: yay("PostgreSQL service stopped")
...
|------|---------------|-------------|
| `playbooks/postgresql-service.sh` | Playbook | Executed via PlaybooksTrait with `DEPLOYER_ACTION=stop` |
...
|-------------|------|------|
| Console | "Stop PostgreSQL Service" heading | Formatted text |
...
| Service state change | Stops PostgreSQL systemd service via `systemctl stop postgresql` |
...
This command is part of the PostgreSQL service control family:

| Command | Action | Playbook Variable |
|---------|--------|-------------------|
| `postgresql:start` | Start service | `DEPLOYER_ACTION=start` |
| `postgresql:stop` | Stop service | `DEPLOYER_ACTION=stop` |
| `postgresql:restart` | Restart service | `DEPLOYER_ACTION=restart` |
| `postgresql:logs` | View logs | Uses `LogsTrait` instead |
| `postgresql:install` | Install PostgreSQL | Uses `postgresql-install` playbook |

### Playbook Behavior

The `postgresql-service.sh` playbook with `DEPLOYER_ACTION=stop`:

1. Validates required environment variables
2. Executes `systemctl stop postgresql` via `run_cmd`
...
- `mysql:stop` - Identical implementation using `mysql-service.sh` playbook
