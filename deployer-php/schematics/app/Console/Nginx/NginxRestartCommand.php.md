# Schematic: NginxRestartCommand.php

> Auto-generated schematic. Last updated: 2025-12-27

## Overview

NginxRestartCommand restarts the Nginx service on a remote server via the nginx-service.sh playbook. It prompts for server selection, executes the playbook with `DEPLOYER_ACTION=restart`, and reports success or failure.
...
```
1. Display header "Restart Nginx Service"
...
3. executePlaybookSilently() [from PlaybooksTrait]
   ...
   |- Message: 'Restarting Nginx service...'
   |- Env: DEPLOYER_ACTION=restart
   |- Failure: Display error, return FAILURE
   +- Success: Continue

4. yay('Nginx service restarted')
...
```mermaid
flowchart TD
    subgraph Command["NginxRestartCommand"]
        exec["execute()"]
        header["h1('Restart Nginx Service')"]
        playbook["executePlaybookSilently()"]
        success["yay('Nginx service restarted')"]
...
| Playbook | Restarts Nginx service via systemctl |

## Notes

- Uses nginx-service.sh playbook with `DEPLOYER_ACTION=restart`
- Playbook verifies Nginx is running after restart (up to 10 second timeout)
