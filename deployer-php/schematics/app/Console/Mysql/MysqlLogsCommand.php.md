# Schematic: MysqlLogsCommand.php

> Auto-generated schematic. Last updated: 2025-12-19 (updated)

## Overview

CLI command that retrieves and displays MySQL service status by fetching logs from two sources: systemd journal logs for the MySQL service and the MySQL error log file. Uses `LogsTrait::retrieveJournalLogs()` and `LogsTrait::retrieveFileLogs()` for log retrieval with error highlighting, and `ServersTrait::selectServerDeets()` for server selection with SSH validation.
...
1. **Display heading** - Output "MySQL Service Status" heading via `h1()`
...
5. **Service log retrieval** - Call `retrieveJournalLogs()` (LogsTrait) with:
   ...
   - Title "MySQL Service" for output heading
   - Unit name "mysql" for systemd journal filtering
   - Line count as integer
6. **Error log retrieval** - Call `retrieveFileLogs()` (LogsTrait) with:
   - Server context for SSH connection
   - Title "MySQL Error Log" for output heading
...
```mermaid
flowchart TD
    subgraph Command["MysqlLogsCommand"]
        A[execute] --> B[h1: MySQL Service Status]
        ...
        G -->|Yes| I[retrieveJournalLogs: MySQL Service]
        I --> I2[retrieveFileLogs: MySQL Error Log]
        I2 --> J[commandReplay]
        J --> K[Return SUCCESS]
    end
    ...
    subgraph LogsTrait["LogsTrait"]
        I --> LT1[h2: MySQL Service]
        LT1 --> LT2[ssh.executeCommand: journalctl -u mysql]
        LT2 --> LT3[highlightErrors]
        LT3 --> LT4[Display journal output]
        I2 --> LT5[h2: MySQL Error Log]
        LT5 --> LT6[ssh.executeCommand: tail /var/log/mysql/error.log]
...
|------|---------------|-------------|
| `app/Services/SshService.php` | Service | Executes `journalctl` command via SSH (through LogsTrait) |
...
| CLI option `--lines` / `-n` | Number of log lines | `string` (no default - omitting triggers prompt with default "50") |
...
| Remote server | MySQL service journal logs | `string` (via SSH) |
| Remote server | MySQL error log file | `string` (via SSH) |

### Outputs

| Destination | Data | Type |
|-------------|------|------|
| Console | MySQL Service Status heading | Formatted text |
| Console | Server details | Formatted key-value pairs |
| Console | MySQL Service journal logs (error-highlighted) | Colored text |
| Console | MySQL Error Log contents (error-highlighted) | Colored text |
...
| Remote command | Executes `journalctl -u mysql -n {lines} --no-pager` on server |
| Remote command | Executes `tail -n {lines} /var/log/mysql/error.log` on server |
...
- **No CLI default for `--lines`**: The option intentionally has no default value. Omitting `--lines` triggers the interactive prompt which has its own default of "50". This follows the non-interactive design pattern where options replace prompts rather than duplicate their defaults.
...
- `mysql:start`, `mysql:stop`, `mysql:restart` - Related MySQL service commands
